...
恒久対策（git worktreeで分ける / Codespaceを分ける / 並行させないと
決める）は未判断。頻度を見てから決める（#176で追跡）。
...
### アクセシビリティの静的レビュー（2026-09-12、#178〜#185、Chat-Ref: CHAT-0913-AR-01）

チャット側で全ページのアクセシビリティ静的レビューを行い、8件を起票した
（コード読解のみによる判定。**実機のスクリーンリーダー・axe-core・
Lighthouseによる検証はしていない**ため、実装前に各issueで実機確認すること）。

| # | 内容 | 再生成 |
|---|---|---|
| #178 | navbar.js の id="navbarDropdown" 7重複・英語aria-label | 不要 |
...
**Lighthouse で拾えていない指摘がある。** #163 の対応後、他ページの
accessibility は 0.98〜1.00 で残指摘は landmark-one-main のみという記録
だが、#178（id重複による誤ったアクセシブルネーム）はその状態でも
存在していた。スコアを到達点として扱わないこと。
