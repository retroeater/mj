...
件数: 211件（open/closed含む）。番号降順。

---

## #211 リポジトリ（issues含む）を private にするかを判断する
...
リポジトリと issues を private にしたいという要望がある（2026-09-13）。実施はしていない。判断に必要な材料をここに集約する。

## 1. GitHub 側の副作用

- public → private にすると、アクセス権のなくなるユーザの star と watcher が完全に削除される。後で public に戻しても復元できない
- 既存の fork は public のまま残り、このリポジトリから切り離される
- issues はリポジトリに属するため自動的に非公開になる
- 既に公開されていた履歴・クローンは消えない。秘匿情報を隠す目的なら private 化だけでは不十分

## 2. 継続するもの

- Cloudflare Workers Builds（GitHub App 連携）。private でも継続する見込みだが、切替後の初回ビルドで check-runs を確認すること
- Codespaces / Claude Code。認証済みのため影響なし（gh・git ともにそのまま）
- チャット側のブラウザ経由の閲覧（Claude for Chrome）。ログイン済みセッションを使うため継続する（2026-09-13 に実測済み）

## 3. 失われるもの（これが主論点）

チャット側（claude.ai）には経路が2つあり、private 化で運命が分かれる。

| 経路 | 認証 | private 化後 |
| --- | --- | --- |
| ブラウザ（Claude for Chrome） | Chrome のセッション cookie | 使える |
| サンドボックス（bash・git・curl） | なし（匿名アクセス） | 使えなくなる |

2026-09-13 の実測結果。

- `git clone --filter=blob:none` は通る。`.git` 812KB、全ブランチ・全履歴が数秒で取得でき、`git log --all --grep`（Chat-Ref の追跡）・`compare`・祖先判定が1コマンドでできる
- `api.github.com` は匿名枠（60回/時・IP共有）のため不安定。実測時は既に使い切られて 403 だった

private 化すると clone が通らなくなる（サンドボックスに GitHub の認証情報はなく、トークンを渡すこともできない）。コミット履歴の横断調査はブラウザで個別ページを開くか、Claude Code に依頼するかになる。

この影響を受けるのは「チャット側が独立に確かめる手段」であって、プロジェクト全体の検証能力ではない（Claude Code 側は無傷）。ただし #169 の教訓と同じ構図で、チャット側が自分で確かめられない領域が広がる。

## 4. 緩和策（未検証を含む）

- 必要なときだけ Codespace で `git bundle create ... --all` を作り、チャットにアップロードする。サンドボックス内で bundle から clone すれば全履歴の調査が戻る（ファイルサイズの実測が必要）
- issue の一括取得が必要な場合も同様に、`gh issue list --json` の出力をその都度アップロードする。リポジトリにコミットしないため、#210 で廃止したエクスポートの問題（再生成漏れ・ズレ・リポジトリの膨らみ）は起きない
- ブラウザで代替できる範囲：コミット検索（Chat-Ref）、`/compare/<base>...<sha>` による先行・遅れと祖先判定（削除済みブランチの SHA でも可）、コミットページの所属ブランチ表示。可否ではなくコストの問題

## 5. 判断すること

- private 化の目的を明確にする（何を隠したいのか）。目的によっては private 化では解決しない
- 上記の検証コストの増加を許容するか、緩和策を先に検証するか

---

起票メモ: チャット側（claude.ai）の Claude がブラウザ経由で作成した。

---

## #210 docs/issues-open.md と docs/issues-snapshot.md の生成をやめる
...
チャット側（claude.ai）はこれまで GitHub を URL 取得でしか参照できなかったため、issue の状況を知る手段として `docs/issues-open.md`（Open のみ）と `docs/issues-snapshot.md`（全件）をリポジトリ内にエクスポートしていた。

2026-09-13、Claude for Chrome（ログイン済みブラウザ）経由で GitHub のページを直接読めることを確認した。Issues 一覧・個別 issue をチャット側から直接参照できるため、エクスポートファイルを介する必要がなくなった。リポジトリを private 化した後もこの経路は使える（private 化は別途検討中）。

エクスポート方式には次の問題がある。

- `issues-snapshot.md` は 620KB あり、チャットで読むと会話が重くなる
- 生成物がリポジトリに入るため、gh issue 操作のたびに再生成 → コミット → push が必要（#140 / #143）
- gh issue 以外の経路（GitHub MCP・gh api・ブラウザ）で操作すると反映されず、実態とズレる
- docs/ 配下が本番配信対象だった問題（#133）で、公開されていたファイルのひとつでもあった

## やること

- [ ] `docs/issues-open.md` / `docs/issues-snapshot.md` を削除
- [ ] `scripts/build_issues_snapshot.py` を削除
- [ ] Claude Code の PostToolUse フック（gh issue 操作時の自動再生成）を外す
- [ ] CLAUDE.md の関連記述を削除
- [ ] docs/handover.md を書き換える（「0. 新しい会話の始め方」「3. 作業の進め方 / タスク管理」「7. 関連文書」）。会話開始時は handover.md → GitHub の Issues 一覧、という順に変更する
- [ ] `.assetsignore` に関連エントリがあれば整理

## 判断が要る点

- クローズ済み issue の追跡をエクスポートなしで回せるか。Claude Code 側は `gh issue list --state all` / `gh issue view` で代替でき、チャット側はブラウザで Issues 一覧（`is:issue is:closed` 等の絞り込み）を開けば足りる想定
- 完全削除ではなく、最終版を1回分だけアーカイブとして残す選択肢もある

## 前提

チャット側がブラウザ経由で GitHub を読める状態であること（2026-09-13 に確認済み）。この前提が崩れると、チャット側は issue の状況を知る手段を失う。
...
#207（規約外`claude/*`ブランチ3本の削除、2026-09-13）で、issueコメントに
記録されたのは「マージ済み → 削除」という判定結果だけで、削除直前の
ブランチ先頭SHAが残っていない。

ブランチを削除するとリモートから参照が消えるため、記録にSHAが無いと:

- 判定が正しかったかを後から検証できない
- 誤って未マージのブランチを削除していた場合に復旧できない

今回は、チャット側が削除前の`git ls-remote`の出力を控えていたため事後に
検証でき、3本とも`cloudflare`の祖先であることを確認できた。つまり
検証できたのは偶然であり、手順として成立していない。

この危険は実在する。同じ日の#206では、指示を書いた時点で
`work/0913-wh2`の先頭は`953c19e`（マージ済み）だったが、実行時には
`c54277c`（#192第1段、未マージ）まで進んでいた。現物を確認せず
削除していれば、コミット1本が失われていた。

## やること

`CLAUDE.md`「ブランチ運用」節に、次の趣旨を追記する:

- ブランチを削除する際は、削除直前の先頭SHAを必ず記録に残すこと。
  記録先は、対応するissueのコメント（issueが無い場合は完了報告）。
  ブランチ名・SHA・マージ済み/未マージの判定を、ブランチごとに1行で書く
...
- 未マージのブランチは削除しない。内容（コミット数・変更ファイル・
  関連issue番号）を記録して平野さんの判断を仰ぐ

あわせてdocs/handover.mdに、#207と#206の経緯を実例として残す。

## 制約

変更はドキュメントのみ。既存のブランチには一切触らない。

Chat-Ref: CHAT-0913-QM-03
...
CLAUDE.mdの「ブランチ運用」節に、削除直前SHAの記録義務・完全な履歴での直前再判定・未マージ時の保留を追記し、docs/handover.mdに#207/#206の実例を残した。work/0913-qmでコミットし`cloudflare`へマージ済み(`8584c39`)。

**retroeater** (2026-09-13):

## 補足: ルールの穴を1件塞いだ

本issueの対応直後、`work/0913-hv`（先頭`feae0e3`）がリモートから消えて
いるのを検出した。`feae0e3`は`cloudflare`の祖先ではなく、一度「未マージ
のまま削除された可能性」と判定した。

**しかし実際には失われていなかった。** 同じ内容が`2deb7d4`として
`cloudflare`に入っていた（件名も同一）。rebase/cherry-pickで積み直され
てからマージされ、元のブランチが削除された、という経緯だった。

祖先関係だけで判定すると、積み直してマージ済みの作業を「失われた」と
誤判定しうる。CLAUDE.mdの「ブランチを削除するときの手順」に以下を追記した:

- 積み直してマージした場合は、元のSHAと`cloudflare`側のSHAの両方を記録する
- 祖先でなかった場合に即「未マージ」と判定せず、件名・差分の突き合わせで
  確認してから判定する

詳細はCLAUDE.md/docs/handover.mdの本コミット参照。
...
2026-09-13、平野さん判断によりA案（セッションごとに作業ブランチを分ける）を採用しました。

- `cloudflare`: 統合・デプロイ専用。セッションは直接pushしない
- `work/<セッション識別子>`: 各セッションの作業ブランチ。自由にpushしてよいが、`cloudflare`へのマージはセッション自身が行わず、平野さんの判断を待つ
- `cloudflare`へのpushで#169のワークフローが動くため、マージ＝本番反映になる
- `git stash`は使用しない（共有パス上で他セッションの未コミット編集を無言で消しうるため）。コミット分割は`git add -p`または`git apply --cached`で行う

CLAUDE.md（冒頭「ブランチ運用」節を新設）・docs/handover.mdに記載しました。
コミット: `0f56958`（ブランチ `work/0913-wh`、push済み）。

このセッション自身も新ルールに従い、`work/0913-wh`ブランチで作業し`cloudflare`へは直接pushしていません。方針決定・文書化が完了したためクローズします。
...
## 確認結果（2026-09-13、平野さんが本番 ryoei.pro で目視確認）

コミット `70a08c8`（class="mj-plain"追加）+ 再生成 `46dff06` の内容を確認。
rh_paifu.htmlの牌譜リンクの下線が消え、色は従来どおり。
...
## 実装の一部がwork/0913-wh2に存在（未マージ、2026-09-13 08:13 UTC確認）

第1段の実装が`work/0913-wh2`（先頭`c54277c`「YouTube動画メタデータの
取得スクリプトを新設する(#192第1段)」）に存在する。`cloudflare`へは
未マージ（`cloudflare`の祖先ではないことを確認済み。同一内容が別SHAで
`cloudflare`に入っていないことも、ファイルの有無とコミット件名で確認済み）。

追加ファイル:

- `scripts/fetch_youtube_meta.py`（188行、新規）
- `.assetsignore`（2行追加）
- `.gitignore`（6行変更）

このブランチの削除可否は`#206`で保留中であり、**#192がマージされるまで
削除しない**という依存関係になっている。

次に#192を引き継ぐ人は、`cloudflare`から新規に始めるのではなく、
`work/0913-wh2`の続きから作業するか、`c54277c`を取り込むかを最初に
判断する必要がある。
...
## コミット
- HV-01（事実修正）: 5129691
- HV-02（分割・肥大化対策）: 67ea3fb
- （付随）issuesスナップショット再生成: 16b5219

## docs/notes/ 4ファイル
- static-generation.md: 241行・18,890 bytes
- video-wayhome.md: 187行・14,589 bytes
- cloudflare.md: 319行・23,027 bytes
- site-findings.md: 308行・20,004 bytes

## 分割後の docs/handover.md
747行・47,769 bytes（上限60KB/900行に対し余裕あり。目安の40KB以下はやや超過）

## 指示と実体が食い違い、指示通りにしなかった/補正した点
- 「選手画像1,985枚は外部7ドメインにあり対象外」は指示では3回出現としていたが、実際は4回（Speed設定表・Cloudflare Proでできること表・検討して見送った技術表2箇所）だった。初出（Speed設定表）のみ残し、残り3箇所を「同上」にした
- 「#7の進め方」の型Aテーブルからvideo_wayhomeを外した際、ページ数(13→12)・完了数(10→9)も内部整合のため合わせて修正した（指示は列挙からの削除のみ言及）
- アクセシビリティ節の5章・6章重複は、本セッション開始前に別セッションのコミット(f609d85)で既に解消済みだった。残っていた「#177もHEAD_TEMPLATEの変更で再生成待ち」の1文（#177は実際にはgenerate_jpml_pros.py専用テンプレートで対応済み・再生成不要）のみ削除した

## #152
resource_efficiency.htmlの見出しフォントサイズ修正は`4738d8d`で対応済み。クローズはせず「状況: 待ち」を付与し、平野さんの目視確認待ちとした。

## #143
実装済み（issues-open.md/build_issues_snapshot.pyで対応済み）のためクローズし、「状況: 保留」ラベルを外した。
...
実機の支援技術での通し確認用チェックリストを用意しました:
`docs/notes/a11y-manual-check.md`

Claude（チャット）が作成した下書きで、平野さんはまだ実施していません。
本文冒頭にその旨を明記しています。A〜E（jpml_pros / video_wayhome /
houou_leagues・ouka_leagues / index.html / 横断確認）のチェック項目と、
NVDAの最低限の操作、記録の残し方をまとめています。
...
実装 `ef1b274`、本番反映 `25e3ae0`（マージ）の内容を確認。

- スマホ幅でハンバーガーボタンの見た目（右上の青い円、40×40px、位置）が変更前と同一
- タップでナビが開閉し、開いたときアイコンが×に変わる
- **マウス／タップでの押下時に白いリングが出ず、キーボードのTabで移動したときだけ出る**（`:focus-visible`が期待どおり動作）
- PC幅でプロフィール画像下の「Ryoei Hirano」の見た目が変わっていない（`<h1>` → `<p class="text-light">`の変更）

## CDPで検証しきれなかった点について

前回コメント時点で、`getComputedStyle`によるoutline幅・オフセットの数値検証が
このセッションの環境（chrome-headless-shell）固有の挙動により確定できていなかった。
今回、平野さんが実機で`:focus-visible`の挙動（ポインタ操作ではリングが出ず、
キーボード操作でのみ出る）そのものを確認できたため、実装した`.mobile-nav-toggle:focus-visible`
のルールが意図どおりに機能していることが確定した。CDP側の数値不一致は環境固有の
表示上の問題であり、実際の挙動には影響していなかったと判断する。
...
コミット `3c9ac7e` の内容を確認。ページ送りで最終ページに着いたあとも
フォーカスが残り、操作不能にならないことを確認。無効状態のボタンの見た目
（薄さ・カーソル）も従来どおり。
...
コミット `e1b352f`（新しいタブ予告の追加）+ jpml_pros B案 `1c296c3` の内容を
確認。見た目に変化なし（予告テキストは画面に出ていない）。
...
コミット `1a689de`（<main>・スキップリンク追加）+ Google Charts 6ページ分
`1ffdcd5` の内容を確認。スキップリンクがフォーカス時に表示され、固定ナビの
下に隠れないことを確認。Google Charts 6ページ（houou_results等）でグラフと
表が従来どおり描画される。
...
コミット `e19863a`（th scope="row"対応）+ リグレッション修正 `bc1fcf8` の内容を確認。
jpml_pros.htmlを横スクロールし、固定された1列目の下を2列目が通るとき文字が
透けないことを確認。縞模様とホバーが1列目にも効いている。選手名の太さ・
中央寄せ・stickyの挙動も従来どおり。ソートと4つの絞り込みも動作。

## リグレッションを出した経緯（今後のための記録）

`<td>`を`<th>`に変えたとき、`.mj-table tbody tr:nth-child(odd) > td`のような
**子セレクタでtdだけを対象にしている既存指定**を見落とすと、その列の背景が
透明になり、sticky列では下の文字が透けて見える不具合につながる。今後
tbodyのtdをthに変える箇所があれば、`> td`を含むセレクタを（`.mj-table`に
限らず）全て洗い出すこと。同種の変更はrh_results_detailの1列目が候補として
残っている（本issue本文に範囲外として記載済み）。
...
`work/0913-ar4`（削除直前の先頭SHA: `570928b`）→ `cloudflare`へfast-forward
マージ（squash/rebaseなし。`cloudflare`のHEAD自体が`570928b`と一致）。
`git merge-base --is-ancestor 570928b HEAD`で確認済み（マージ済み）。
リモート・ローカルとも削除済み。
...
コミット `3c9ac7e`（初回実装）+ `381cbcb`（body→html修正）の内容を確認。
`jpml_pros.html` で Tab を連打し、表内のリンクへフォーカスが移った際に
フォーカスリングが固定ナビ・stickyヘッダーの下に隠れないことを確認。
検索欄を開いた状態（`--content-offset` が増える）でも隠れない。

`body` に書くと効かなかった経緯は前回コメント
（https://github.com/retroeater/mj/issues/179#issuecomment-5650501494）に
記録済み。
...
コミット `3c9ac7e` の内容を確認。ナビの各ドロップダウンが開閉できることを確認
（クリック・Enter・Escape）。見た目に変化なし。
...
実機確認完了。iOSのSafariで jpml_pros.html から「ホーム画面に追加」を
開き、名前欄の既定値が「連盟プロ」になっていることを確認した。

反映コミット: f382109（PAGE_TEMPLATEへのmeta追加）/ 51e5923（再生成）

### 確認時に分かったこと

iOS 17以降、「ホーム画面に追加」ダイアログに「Webアプリとして開く」
トグルがあり、既定でオンになっている。apple-mobile-web-app-capable を
宣言していなくてもオンで表示されるため、スタンドアロン起動にするか
どうかは実質ユーザー側の選択で、宣言でオフに倒すことはできない。

オンのまま追加して動作を確認したところ、jpml_pros.html の検索・ソート・
外部リンク（X・note・YouTube）はいずれも正常に動作した。対応は不要。
handover.md に記録済み（06331b9）。

---

## #176 ブランチ運用ルールの実効性を監視し、強制策が必要かを判断する
...
### スコープ変更（2026-09-13、#205/#209を受けて）

当初の目的は#198で決着した。採用されたのは`git worktree`によるツリー分離
（「検討する対策案」の1つ目）。上記の本文・観測された影響2件は**旧スコープの
記録**として保持する。

新しいスコープ: **ブランチ運用ルールのすり抜け・違反を記録し、強制策
（自動検知）が必要かを判断する。**

決まった運用は規約でしかなく、強制する仕組みが無い。実際、ルールが入った
当日に`1b8eec6`が`work/`ブランチを経由せず`cloudflare`へ直接pushされている
（#205）。いま薄いのは「対策が決まっていないこと」ではなく「決まった対策が
守られる保証が無いこと」。

**記録対象とする違反:**

- `cloudflare`への直接コミット・push
- `work/<識別子>`以外の命名の作業ブランチ
- `/workspaces/mj`でのブランチ切り替え（worktreeを使わない作業）
- `git stash`の使用
- ブランチ削除時にSHAを記録しない（#209）

**記録の書式（1件につきコメント1つ）:** 日付 / Chat-Ref / 該当コミットまたは
ブランチ / 破られたルール / 実害の有無と内容 / どうやって気づいたか

**判断の目安:** 実害を伴う違反が1件出た時点、または実害の無い違反が3件
たまった時点で、強制策の検討へ進む。

**強制策の候補（判断材料。ここでは決定しない）:**

- `pre-push`フックでの検知
- GitHub Actionsでの直接push検知
...
- 日付: 2026-09-13
- Chat-Ref: CHAT-0913-QM-01
- 該当コミット: `1b8eec6`
- 破られたルール: 「`cloudflare`: 統合・デプロイ専用。セッションはここへ
  直接pushしない」
- 実害: 無し（ドキュメントのみの変更で、内容は正しかった）
- 気づいた経緯: チャット側が完了確認で`git log`の親コミットを見て検出
- 補足: チャット側の指示文が旧前提のままで作業ブランチの指定を欠いて
  いたことが一因（#205で対処済み）
...
iOSでホーム画面に追加すると、紺の丸の背後に黒い四隅が見える。

## 原因

apple-touch-icon.png（180×180 RGBA）は紺(#001E62)の円＋白いRで、四隅が
完全透過（32,400px中6,536pxがアルファ0）。iOSはホーム画面アイコンの
アルファチャンネルを扱わず、透過部分を黒で合成する仕様のため。
角丸マスクはiOS側が後から掛ける。

**透過を保ったまま黒を消すことはできない。** 四隅まで不透明にするのが
唯一の解。

## 対応

円を四隅まで紺で伸ばした不透明版に差し替える（白いRの見え方は現状と
同一で、黒だけが消える。円の輪郭はiOSの角丸マスクが代わりを果たす）。

favicon.ico は変更しない。ブラウザのタブは透過を正しく扱うため、
丸いまま表示されるのが正しい挙動。

## 併せて対応（キャッシュ）

_headers の /apple-touch-icon.png と /favicon.ico が
max-age=31536000, immutable のため、差し替えても既存訪問者には最大1年
古いアイコンが残る。アセット名にハッシュを持たない構成なので、
assets/vendor（30日）と同じ落とし穴。max-age=86400 に緩和する。

## 確認方法

すでにホーム画面に追加済みのアイコンは、追加時点の画像が焼き付くため
差し替わらない。検証は一度削除してから再追加する。

## スコープ外（別途判断）

- 全ページへの <link rel="apple-touch-icon"> の明示と
  apple-mobile-web-app-title の追加。HEAD_TEMPLATE の変更になり
  全26ページ＋wayhome 38枚の再生成が必要なため、別の再生成が発生する
  タイミングに合わせる
...
着手中: apple-touch-icon.png の透過をやめ、iOSホーム画面の黒い四隅を解消する(#174)。
apple-touch-icon.png は差し替え済み（180×180・RGB・アルファなし確認済み）。
...
対応済みのため、状況を整理してクローズします。

- 修正コミット `d310b03`（cloudflareに反映済み）で、`regenerate-page.yml` の差分検知を
  push範囲全体（`github.event.before` 〜 `github.sha`）で取るように変更済み
- `fetch-depth: 0`、範囲が特定できない場合（ブランチ新規作成・force push・before空文字）の
  `all` フォールバック、`|| true` の除去（失敗を握りつぶさない）まで対応済み
- CLAUDE.md にも「検知はpushに含まれる全コミットの範囲…で行う（#167。以前は最終コミット
  1つ分しか見ておらず…）」として反映済み

コメント内の「残る確認」（該当ファイルを含むpushで新しい経路が実走するか）については、
その後の通常運用のpushで `regenerate-page.yml` が動いており問題は再発していないため、
これ以上待たずにCOMPLETEDでクローズします。
...
本番検証3点、平野さんのスクリーンショットで確認いただきました（2026-09-12）。クローズします。

### 本番検証結果（2026-09-12、平野さん確認）

1. `/wayhome/UtxpVoWy2GY.html` の表示OK
2. GSCサイトマップ: `sitemap.xml`が「サイトマップ インデックス」として成功、`sitemap-pages.xml`25件・`sitemap-wayhome.xml`38件ともに成功。インデックス行の「検出されたページ数 0」はGSCの仕様どおりで正常（配下の子サイトマップ側で計上される）
3. リッチリザルトテスト: `/wayhome/UtxpVoWy2GY.html`で`VideoObject`1件が有効（#13の「`ItemList`の`url`がyoutube.comのため対象外」だった状態が、個別ページでは自サイトURLを指すことで解消した）

### 途中で起きた問題

`sitemap-pages.xml`冒頭のXMLコメントに含まれていた`--`（`regenerate.py --list`の記述、#121由来で#162より前から存在）がGSCで「型: 不明 / 1件のエラー」を引き起こしていた。コメントから`--`を除去し、再発防止として`regenerate-page.yml`と`scripts/generate_wayhome_episodes.py`の両方にXMLのwell-formedness確認を追加した。詳細はdocs/handover.mdのSEO節。

### 選手個別ページ（#101 / #159）へ持ち越す4点

1. **相対パス前提のテンプレートはサブディレクトリ1枚目で壊れる。** `lib/page.py`の`asset_prefix`引数とnavbar.jsのルート相対href化で対応したが、新サイトではルーティングの機構自体で吸収する設計にすべき
2. **サイトマップは手書きせず生成物にし、インデックス方式で分割する。** ページの生成スクリプトがサイトマップの該当行も一緒に面倒を見る構成にしておく
3. **手書きコメントの`--`のような小さな構文違反でファイル全体が読まれなくなる。** XMLはこの種の弱さがあるため、well-formednessチェックをCIに機械的に置く
4. **個別ページのJSON-LDは自サイトURLを指せば初めてリッチリザルトの対象になる。** 構文が正しいだけでは不十分で、ページ自身のURLを指していることが前提

docs/new-site-design.md §12「#162で確かめたこと」に同じ内容を記録済み。
...
本番目視確認済み（2026-09-13、平野さん）。対応コミット 4738d8d
...
クローズ済みだが「状況: 保留」ラベルが残っていたため外す。CLAUDE.mdの規約（#112由来）は当初「状況: 待ち」「状況: 対応中」のみを対象にしていたが、対象を「状況:」ラベル全体に広げた（このissueは保留中ではなく、対応または判断を経て閉じたもの）。

Chat-Ref: CHAT-0913-K7-04
...
#162（エピソード個別ページ38枚の静的生成）で、この判断の例外として38ページにだけ`<link rel="canonical">`を付けた。

理由: 本issueが「canonicalなし」と判断した根拠は、`?name=`付きURLの検索流入（14件）を正規化で失うことだった。エピソード個別ページ（`wayhome/<動画ID>.html`）は`?name=`等のURL変種を持たないため、この懸念がそもそも当てはまらない。`lib/page.py`の`PageMeta.canonical`（既定`None`）で、ページごとにcanonicalの有無を切り替えられるようにしてある。

他27ページはcanonicalなしのまま変更していない。詳細はdocs/handover.mdの「#162 エピソード個別ページ38枚」節・CLAUDE.mdの該当箇所に記録済み。
